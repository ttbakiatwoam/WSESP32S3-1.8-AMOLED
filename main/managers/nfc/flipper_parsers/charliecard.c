...
// starts Wednesday 2003/1/1 @ midnight (weekday ignored in compat build)
#define CHARLIE_EPOCH \
    (DateTime){ .year = 2003, .month = 1, .day = 1, .hour = 0, .minute = 0, .second = 0 }
...
uint32_t time_now() {
    // compat shim: reuse DateTime timestamp based on current RTC if available.
    // For now, just return 0; CharlieCard uses this for relative comparisons only.
    return 0;
}

static bool is_debug() {
    // compat shim: always false (no debug flag support yet)
    return false;
}
...
static bool charliecard_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    do {
        ...
        furi_string_cat_printf(parsed_data, "CharlieCard");
...
const NfcSupportedCardsPlugin charliecard_plugin = {
