...
static bool troika_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    do {
        ...
        // For now we omit the detailed mosgortrans history parsing and just
        // indicate that a Troika card was detected.
        furi_string_cat_printf(parsed_data, "Troyka card\n");
        parsed = true;
    } while(false);
...
const NfcSupportedCardsPlugin troika_plugin = {
