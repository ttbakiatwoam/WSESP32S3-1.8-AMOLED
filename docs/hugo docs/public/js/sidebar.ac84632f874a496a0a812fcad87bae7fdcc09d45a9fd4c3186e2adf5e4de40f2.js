document.addEventListener('DOMContentLoaded', function() {
  ...
  if (versionSelect) {
    ...
    if (storedVersion && (!match || storedVersion !== match[1])) {
      const targetPath = match && match[2] ? `/docs/${storedVersion}${match[2]}` : `/docs/${storedVersion}/`;
    ...
    versionSelect.addEventListener('change', () => {
      ...
      const current = window.location.pathname.match(/^\/docs\/([^\/]+)(\/.*)?$/);
      const suffix = current && current[2] ? current[2] : '/';
      const destination = `/docs/${newVersion}${suffix}`.replace(/\/+$/, '/');
