...
// Asset lookup constants removed (not used in GhostESP build)
...
static void fill_name(uint16_t id, FuriString* name) {
    switch(id) {
    case 0x0000:
...
static bool skylanders_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    do {
        ...
        furi_string_printf(
            ...
            "Skylanders\nID: 0x%04X\nName: %s",
            id,
            name ? furi_string_get_cstr(name) : "Unknown");
...
const NfcSupportedCardsPlugin skylanders_plugin = {
