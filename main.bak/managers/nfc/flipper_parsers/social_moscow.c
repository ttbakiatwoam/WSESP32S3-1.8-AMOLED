...
// --------------------------------------------------------------------------
// Minimal stubs for Flipper mosgortrans utilities
// --------------------------------------------------------------------------

static bool mosgortrans_parse_transport_block(const MfClassicBlock* block, FuriString* out) {
    (void)block;
    (void)out;
    // Detailed Moscow transport history parsing is not yet implemented in GhostESP.
    return false;
}

static void render_section_header(FuriString* out, const char* title, uint8_t width, uint8_t pad) {
    (void)width;
    (void)pad;
    if(!out || !title) return;
...
static bool social_moscow_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    do {
        ...
        if(is_metro_data_present && metro_result && furi_string_get_cstr(metro_result)[0] != '\0') {
            render_section_header(parsed_data, "Metro", 22, 21);
            furi_string_cat_printf(parsed_data, "%s\n", furi_string_get_cstr(metro_result));
        }
        if(is_ground_data_present && ground_result && furi_string_get_cstr(ground_result)[0] != '\0') {
...
const NfcSupportedCardsPlugin social_moscow_plugin = {
